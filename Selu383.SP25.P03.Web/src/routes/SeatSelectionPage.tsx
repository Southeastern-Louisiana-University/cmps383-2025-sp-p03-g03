...
export default function SeatSelection() {
  ...
  const generateTestSeats = (roomId: number): Seat[] => {
    const seats: Seat[] = [];
    const rows = ["A", "B", "C", "D", "E", "F", "G", "H"]; // Extended to 8 rows for variety
    const seatsPerRow = roomId === 14 ? 10 : roomId === 1 ? 20 : 15; // Small, Large, Medium theaters

    rows.forEach((row, rowIndex) => {
      for (let i = 1; i <= seatsPerRow; i++) {
        let seatTypeId: number;

        // Assign seat quality based on row position and theater size
        if (roomId === 14) {
          // Small theater: All seats are VIP for exclusivity
          seatTypeId = 5; // VIP
        } else if (roomId === 1) {
          // Large theater: Diverse seat quality
          if (rowIndex === 0 || rowIndex === 1) {
            seatTypeId = 5; // VIP: Front rows for premium experience
          } else if (rowIndex === 2 || rowIndex === 3) {
            seatTypeId = 2; // Premium: Mid-front rows
          } else if (rowIndex === 4 && (i === 1 || i === seatsPerRow)) {
            seatTypeId = 4; // Accessible: Aisle seats in middle row
          } else if (rowIndex >= 6) {
            seatTypeId = 3; // Recliner: Back rows for comfort
          } else {
            seatTypeId = 1; // Standard: Remaining seats
          }
        } else {
          // Medium theater (default, e.g., roomId === 7)
          if (rowIndex === 0) {
            seatTypeId = 5; // VIP: Front row
          } else if (rowIndex === 1 || rowIndex === 2) {
            seatTypeId = 2; // Premium: Next rows
          } else if (rowIndex === 3 && (i <= 2 || i >= seatsPerRow - 1)) {
            seatTypeId = 4; // Accessible: Edge seats in row D
          } else if (rowIndex >= 5) {
            seatTypeId = 3; // Recliner: Back rows
          } else {
            seatTypeId = 1; // Standard: Remaining seats
          }
        }
        ...
        seats.push({
          ...
          seatTypeId, // Reflects seat quality: 1=Standard, 2=Premium, 3=Recliner, 4=Accessible, 5=VIP
          roomsId: roomId,
          isAvailable: Math.random() > 0.3, // Random availability (70% chance available)
          row,
          seatNumber: i,
          xPosition: i * (roomId === 14 ? 60 : 40), // Spacing adjusted for theater size
          yPosition: (rowIndex + 1) * (roomId === 14 ? 80 : 40), // Vertical spacing
        });
      }
    });
