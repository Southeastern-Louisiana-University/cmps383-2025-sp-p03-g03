    ...
    const fetchData = async () => {
      try {
        ...
        const [types, seatsData, takenSeatsData] = await Promise.all([
          SeatTypeService.getAll(),
          SeatService.getByRoomId(Number(roomId)),
          SeatTakenService.getBySchedule(
            Number(theaterId),
            Number(scheduleId),
            Number(roomId)
          ),
        ...
        const updatedSeats = seatsData.map((seat) => {
          const isTaken = Array.isArray(takenSeatsData)
            ? takenSeatsData.some(
                (takenSeat) => takenSeat.seatId === seat.id && takenSeat.isTaken
              )
            : false;
                          ...
                          className={`absolute rounded-full flex items-center justify-center transition-all duration-300 cursor-pointer
                            ${
                              !seat.isAvailable
                                ? unavailableColor
                                : isSelected
                                ? `${selectedColor} ${accessibleRing}`
                                : `${baseColor} ${accessibleRing}`
                            }
          ...
          {seatTypes.map((type) => (
            <div key={type.id} className="flex items-center gap-2">
              <div
                ...
                } ${type.id === 4 ? "ring-3 ring-yellow-400" : ""}`}
              ></div>
