...
interface TicketDto {
  ticketId?: number; // Optional, assigned by backend
  orderId: number;
  screeningId: number;
  seatId: number | null;
...
export default function Checkout() {
  ...
  const handlePayment = async () => {
    ...
    try {
      const orderId = Date.now(); // Temporary unique order ID
      ...
      for (const seat of selectedSeats) {
        const ticket: TicketDto = {
          orderId,
          screeningId: showtime.id, // Assumes showtime.id is the screening ID
          seatId: seat.id,
          ticketType: getSeatType(seat.seatTypeId),
        ...
        if (!ticketResponse.ok) {
          ...
          throw new Error(
            errorData.message || `Failed to create ticket for seat ${seat.id}`
          );
        }

        const createdTicket: TicketDto = await ticketResponse.json();
        createdTicketIds.push(createdTicket.ticketId!);
      }

      // Create UserTicket entries for each ticket
      for (const ticketId of createdTicketIds) {
        const userTicket: UserTicketDto = {
          userId: parseInt(userId),
          ticketId,
        };

        const userTicketResponse = await fetch("/api/userticket", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${localStorage.getItem("token")}`,
        ...
        if (!userTicketResponse.ok) {
          const errorData = await userTicketResponse.json();
          throw new Error(
            errorData.message || `Failed to link ticket ${ticketId} to user`
          );
        }
      }
